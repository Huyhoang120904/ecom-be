...
The mappers return payloads only. Wrapping in ``BaseResponse`` happens at the route,
where the status code is written out, so the envelope cannot disagree with the status
the route declared.
"""
...
from app.api.media_urls import avatar_url, background_url
...
from app.services.identity_service import Session
