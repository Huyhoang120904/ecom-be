...
class TestUsers:
    async def test_finds_a_user_regardless_of_email_case(self, db_session):
        ...
        found = await repository.find_user_by_email(db_session, "A@EXAMPLE.COM")
    ...
    async def test_a_soft_deleted_user_is_invisible_to_the_lookup(self, db_session):
        ...
        await repository.soft_delete_user(db_session, user)

        assert await repository.find_user_by_email(db_session, "a@example.com") is None
    ...
    async def test_soft_deleting_releases_the_email_for_reuse(self, db_session):
        ...
        await repository.soft_delete_user(db_session, first)
    ...
    async def test_clearing_a_nullable_field_is_distinct_from_leaving_it(
        ...
        user = await _make_user(db_session)
        await repository.update_user_profile(db_session, user, {"bio": "Lamps."})

        await repository.update_user_profile(db_session, user, {"bio": None})
    ...
    async def test_deactivation_does_not_release_the_email(self, db_session):
        ...
        await repository.deactivate_user(db_session, user)

        assert user.deactivated_at is not None
        assert user.deleted_at is None
        assert (
            await repository.find_user_by_email(db_session, "keep@example.com")
        ) is not None


class TestShops:
    async def test_suffixes_a_colliding_slug(self, db_session):
        first = await repository.create_shop(db_session, name="Hoang Goods")
        second = await repository.create_shop(db_session, name="Hoang Goods")
    ...
    async def test_a_soft_deleted_slug_does_not_force_a_suffix(self, db_session):
        ...
        first = await repository.create_shop(db_session, name="Reused Name")
        await repository.soft_delete_shop(db_session, first)

        second = await repository.create_shop(db_session, name="Reused Name")

        assert second.slug == "reused-name"

    async def test_slugifies_a_non_ascii_name(self, db_session):
        shop = await repository.create_shop(db_session, name="Cửa hàng Hoàng")

        assert shop.slug == "cua-hang-hoang"

    async def test_a_shop_update_never_touches_the_slug(self, db_session):
        shop = await repository.create_shop(db_session, name="Original Name")

        await repository.update_shop_profile(
    ...
    async def test_an_operator_can_suspend_a_shop(self, db_session):
        shop = await repository.create_shop(db_session, name="Suspendable")

        await repository.set_shop_active(db_session, shop.id, False)
...
class TestMembershipsAndPermissions:
    async def test_resolves_the_role_and_its_permission_keys(self, db_session):
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="Perm Shop")
    ...
    async def test_a_viewer_resolves_fewer_keys(self, db_session):
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="Viewer Shop")
        viewer = await repository.get_role_by_key(db_session, "viewer")
    ...
    async def test_no_membership_resolves_to_none(self, db_session):
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="Unrelated")

        assert (
            await repository.effective_permissions(
                db_session, user_id=user.id, shop_id=shop.id
            )
        ) is None
    ...
    async def test_a_revoked_membership_takes_effect_immediately(self, db_session):
        ...
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="Revoked Shop")
        owner = await repository.get_owner_role(db_session)
    ...
    async def test_a_deactivated_user_resolves_to_none(self, db_session):
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="Deact Shop")
        owner = await repository.get_owner_role(db_session)
        await repository.create_membership(
            db_session, user_id=user.id, shop_id=shop.id, role_id=owner.id
        )
    ...
    async def test_a_deleted_shop_makes_its_memberships_unresolvable(self, db_session):
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="Gone Shop")
        owner = await repository.get_owner_role(db_session)
        await repository.create_membership(
            db_session, user_id=user.id, shop_id=shop.id, role_id=owner.id
        )

        await repository.soft_delete_shop(db_session, shop)
    ...
    async def test_memberships_are_listed_oldest_first(self, db_session):
        ...
        for name in ("First Shop", "Second Shop", "Third Shop"):
            shop = await repository.create_shop(db_session, name=name)
            await repository.create_membership(
                db_session, user_id=user.id, shop_id=shop.id, role_id=owner.id
            )

        memberships = await repository.list_memberships(db_session, user.id)
    ...
    async def test_demoting_a_membership_changes_the_resolved_keys(self, db_session):
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="Demote Shop")
        owner = await repository.get_owner_role(db_session)
        await repository.create_membership(
            db_session, user_id=user.id, shop_id=shop.id, role_id=owner.id
        )
...
class TestRefreshTokens:
    async def _issue(self, db_session, user, shop, raw: str = "opaque-token"):
        return await repository.create_refresh_token(
            db_session,
            user_id=user.id,
    ...
    async def test_a_token_is_found_by_its_digest(self, db_session):
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="Token Shop")
        issued = await self._issue(db_session, user, shop)

        found = await repository.find_refresh_token(
    ...
    async def test_an_unknown_digest_finds_nothing(self, db_session):
        assert (
            await repository.find_refresh_token(db_session, hash_refresh_token("nope"))
        ) is None

    async def test_the_raw_token_is_never_stored(self, db_session):
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="Raw Shop")
    ...
    async def test_revoking_a_family_revokes_only_that_family(self, db_session):
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="Family Shop")
        first = await self._issue(db_session, user, shop, "one")
        other_family = await repository.create_refresh_token(
            db_session,
            user_id=user.id,
            active_shop_id=shop.id,
        ...
        revoked = await repository.revoke_family(db_session, first.family_id)

        assert revoked == 1
        assert await repository.list_unrevoked_family(db_session, first.family_id) == []
        assert (
            len(
                await repository.list_unrevoked_family(
    ...
    async def test_revoking_every_family_for_a_user(self, db_session):
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="All Shop")
        await self._issue(db_session, user, shop, "a")
        await self._issue(db_session, user, shop, "b")

        revoked = await repository.revoke_all_families_for_user(db_session, user.id)

        assert revoked == 2

    async def test_revoking_the_families_bound_to_a_shop(self, db_session):
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="Bound Shop")
        other = await repository.create_shop(db_session, name="Elsewhere Shop")
        ...
        revoked = await repository.revoke_families_for_shop(db_session, shop.id)

        assert revoked == 1

    async def test_expired_tokens_are_pruned_lazily(self, db_session):
        user = await _make_user(db_session)
        shop = await repository.create_shop(db_session, name="Expiry Shop")
        stale = await repository.create_refresh_token(
            db_session,
            user_id=user.id,
            active_shop_id=shop.id,
        ...
        deleted = await repository.delete_expired_refresh_tokens(db_session)

        assert deleted >= 1
        assert await repository.find_refresh_token(db_session, stale.token_hash) is None
